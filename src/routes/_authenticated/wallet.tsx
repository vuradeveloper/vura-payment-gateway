...
function WalletPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-8 py-8 md:py-10">
      <PageHeader eyebrow="Money" title="Wallet" description="Top up Vura cash and manage payment methods." />

      <Card className="rounded-2xl border-0 p-6 shadow-soft text-primary-foreground mb-8" style={{ background: "var(--gradient-vura)" }}>
        <p className="text-xs uppercase tracking-wider opacity-80">Vura cash</p>
        <p className="text-5xl font-bold mt-2">R {Number(profile?.wallet_balance ?? 0).toFixed(2)}</p>
      ...
      <div className="space-y-3 mb-8">
        {(methods ?? []).map((m) => (
          <Card key={m.id} className="rounded-2xl border p-5 shadow-card flex items-center gap-4">
            <div className={`w-11 h-11 rounded-lg flex items-center justify-center ${m.type === "cash" ? "bg-emerald-50 text-emerald-600" : "bg-blue-50 text-blue-600"}`}>
              {m.type === "cash" ? <Banknote className="h-5 w-5" /> : <CreditCard className="h-5 w-5" />}
            </div>
            <div className="flex-1">
              <p className="font-semibold">{m.label}</p>
              <p className="text-xs text-muted-foreground">
                {m.type === "card" ? `•••• ${m.last4} · Expires ${m.expiry}` : "Pay the driver at the end of the trip"}
              </p>
            </div>
            {m.is_default && <span className="text-[10px] uppercase font-bold tracking-wider text-primary bg-primary-soft px-2 py-1 rounded-full">Default</span>}
          </Card>
        ))}
        {!methods?.length && (
      ...
      <Button variant="outline" className="w-full h-12 border-dashed border-primary/40 text-primary hover:bg-primary-soft" onClick={async () => {
        const { data: u } = await supabase.auth.getUser();
        if (!u.user) return;
        const { error } = await supabase.from("payment_methods").insert({
          user_id: u.user.id, type: "cash", label: "Cash", is_default: !methods?.length,
        });
        if (error) return toast.error(error.message);
